...
- Wraps `WorldModel` and `QubeRuntime`
- `observe()`: Returns tiles, avatar position, qube state hash
- `act(token)`: Translates token to `SpawnPlane` or `DockPattern` calls
- `get_state_schema()`: Defines expected state structure

**Integration Points**:
- Uses existing `WorldModel.GetTiles()`, `GetSpawnPoint()`
- Uses existing `QubeRuntime.DockPattern()`, `GetStateHash()`
